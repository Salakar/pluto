...
template <typename Record> pluto_pen_ring_header *SpscRing<Record>::header() {
...
template <typename Record> Record *SpscRing<Record>::records() {
  return records_after<Record>(header());
}

template <typename Record> const Record *SpscRing<Record>::records() const {
  return records_after<Record>(header());
}

template <typename Record> void SpscRing<Record>::reset() {
  pluto_pen_ring_header *h = header();
...
template <typename Record> void SpscRing<Record>::push(const Record &record) {
  pluto_pen_ring_header *h = header();
