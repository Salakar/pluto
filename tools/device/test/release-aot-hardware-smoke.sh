...
[[ -f "$PYTHON_ENTRY" && -x "$PYTHON_ENTRY" ]] || {
  echo "release AOT smoke: pinned Python interpreter is unavailable: $PYTHON_ENTRY" >&2
  exit 66
}
PYTHON_BIN="$("$PYTHON_ENTRY" -I -c \
  'import os, sys; print(os.path.realpath(sys.executable))')" || {
  echo "release AOT smoke: cannot resolve pinned Python interpreter: $PYTHON_ENTRY" >&2
  exit 66
}
[[ "$PYTHON_BIN" == /* && "$PYTHON_BIN" != *$'\n'* &&
  "$PYTHON_BIN" != *$'\t'* && -f "$PYTHON_BIN" &&
  ! -L "$PYTHON_BIN" && -x "$PYTHON_BIN" ]] || {
  echo "release AOT smoke: pinned Python interpreter resolved to an unsafe executable: $PYTHON_BIN" >&2
  exit 66
}
