...
def draw_scene(
    ...
    *,
    ...
    if stage in (4, 7):
        marker_x = 0.84 if stage == 4 else 0.90
    ...
    should_stroke = stage == 9 if stroke is None else stroke
    if should_stroke:
...
class VisualPixelsTest(unittest.TestCase):
    ...
    def test_accepts_aligned_cross_modal_set_and_emits_json(self) -> None:
        ...
        self.assertEqual(payload["stages"], 11)
    ...
    def test_allows_codex_equivalent_frames(self) -> None:
        swap_camera_stages(self.root, 4, 7)
        result = self.run_verifier()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("PASS stages=11", result.stdout)
    ...
    def test_rejects_reencoded_decoded_noop(self) -> None:
        before_native = self.root / "native" / f"{LABELS[8]}.png"
        replace_stage(
            self.root,
            "native",
            9,
        ...
        after_native = self.root / "native" / f"{LABELS[9]}.png"
    ...
    def test_rejects_global_exposure_change(self) -> None:
        replace_stage(self.root, "native", 9, exposure=-24)
        replace_stage(self.root, "camera", 9, exposure=-24)
        self.assert_fails("global exposure change")

    def test_rejects_global_blob(self) -> None:
        replace_stage(self.root, "native", 9, blob=True)
        replace_stage(self.root, "camera", 9, blob=True)
        result = self.run_verifier()
        self.assertNotEqual(result.returncode, 0)
        self.assertRegex(
            result.stderr,
    ...
    def test_rejects_wrong_corridor(self) -> None:
        replace_stage(self.root, "native", 9, curve_offset=0.13)
        replace_stage(self.root, "camera", 9, curve_offset=0.13)
        result = self.run_verifier()
        self.assertNotEqual(result.returncode, 0)
