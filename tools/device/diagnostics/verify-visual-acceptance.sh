...
decode_image() {
  ...
  command -v ffmpeg >/dev/null 2>&1 ||
    die "ffmpeg is required to decode acceptance images"
  ffmpeg -v error -nostdin -i "$image" -map 0:v:0 -f null - \
...
image_dimensions() {
  ...
  command -v ffprobe >/dev/null 2>&1 ||
    die "ffprobe is required to inspect acceptance image dimensions"
...
pixel_difference() {
  ...
  local value
  command -v ffmpeg >/dev/null 2>&1 ||
    die "ffmpeg is required for decoded acceptance image comparison"
...
while IFS=$'\t' read -r key value extra; do
  ...
  case "$key" in
    ...
    device_endpoint)
      python3 "$ACCEPTANCE_IDENTITY" validate-endpoint --endpoint "$value" \
...
[[ "${metadata_values[9]}" == 0 || "$ALLOW_TEST_EVIDENCE" == 1 ]] ||
  die "test-seam evidence cannot satisfy production visual acceptance"
...
grep -Eq "\"gitRevision\"[[:space:]]*:[[:space:]]*\"${metadata_values[0]}\"" \
  "$release_manifest" || die "frozen release manifest revision does not match metadata"
grep -Eq "\"${metadata_values[3]}\"[[:space:]]*:" "$release_manifest" ||
...
[[ "$camera_provenance_index" == 16 ]] ||
  die "camera provenance must contain exactly 16 ordered rows"
...
if [[ "${metadata_values[9]}" == 0 ]]; then
  ...
  [[ -f "${camera_provenance_values[12]}" &&
    ...
    "${camera_provenance_values[13]}" =~ ^[0-9a-f]{64}$ &&
    "$(sha256_file "${camera_provenance_values[12]}")" == "${camera_provenance_values[13]}" ]] ||
    die "camera provenance config path or digest is stale"
  [[ "${camera_provenance_values[14]}" == camera-config.json &&
    "${camera_provenance_values[15]}" == "${camera_provenance_values[13]}" ]] ||
    die "camera provenance config snapshot contract is invalid"
  config_snapshot="$CAMERA_DIR/${camera_provenance_values[14]}"
  [[ -f "$config_snapshot" && ! -L "$config_snapshot" &&
    "$(sha256_file "$config_snapshot")" == "${camera_provenance_values[15]}" ]] ||
    die "frozen camera config snapshot is missing or stale"
  python3 "$ACCEPTANCE_IDENTITY" camera-profile --config "$config_snapshot" \
...
grep -Fq '"format": "pluto-acceptance-manifest-proof"' "$proof" ||
  die "installed-byte manifest proof format is invalid"
grep -Eq "\"gitRevision\"[[:space:]]*:[[:space:]]*\"${metadata_values[0]}\"" \
  "$proof" || die "installed-byte proof revision does not match metadata"
grep -Eq "\"manifestSha256\"[[:space:]]*:[[:space:]]*\"${metadata_values[2]}\"" \
  "$proof" || die "installed-byte proof manifest digest does not match metadata"
grep -Eq "\"target\"[[:space:]]*:[[:space:]]*\"${metadata_values[3]}\"" \
  "$proof" || die "installed-byte proof target does not match metadata"
installed_file_count="$(sed -n 's/^[[:space:]]*"installedFileCount"[[:space:]]*:[[:space:]]*\([0-9][0-9]*\),\{0,1\}[[:space:]]*$/\1/p' "$proof")"
[[ "$installed_file_count" =~ ^[0-9]+$ ]] && ((installed_file_count >= 30)) ||
  die "installed-byte proof has an implausible file count"
grep -Fq '"status": "PASS"' "$proof" || die "installed-byte proof did not pass"
summary="$metrics_dir/summary.txt"
[[ "$(sed -n 's/^git_revision=//p' "$summary")" == "${metadata_values[0]}" &&
  "$(sed -n 's/^profile=//p' "$summary")" == "${metadata_values[1]}" &&
  "$(sed -n 's/^target=//p' "$summary")" == "${metadata_values[3]}" &&
  "$(sed -n 's/^local_manifest=//p' "$summary")" == "${metadata_values[2]}" &&
  "$(sed -n 's/^status=//p' "$summary")" == PASS ]] ||
  die "metrics summary does not match the accepted release"
[[ "$(tail -n 1 "$metrics_dir/device-evidence.txt")" == collection.status=PASS ]] ||
...
if [[ "${metadata_values[9]}" == 0 ]]; then
  ...
  [[ -x "$dart" && -f "$packages" && -f "$MANIFEST_VERIFIER" &&
    ! -L "$MANIFEST_VERIFIER" ]] ||
    die "pinned installed-byte proof verifier is unavailable"
  recomputed_proof="$(mktemp "${TMPDIR:-/tmp}/pluto-manifest-proof.XXXXXX")"
  ...
  cmp -s "$recomputed_proof" "$proof" ||
    die "stored installed-byte proof differs from a fresh pinned-Dart proof"
  rm -f "$recomputed_proof"
  trap - EXIT
fi

[[ "$(wc -l < "$camera_manifest" | tr -d '[:space:]')" == 11 ]] ||
  die "camera manifest must contain exactly 11 rows"
[[ "$(wc -l < "$screenshot_manifest" | tr -d '[:space:]')" == 11 ]] ||
  die "screenshot manifest must contain exactly 11 rows"
[[ "$(wc -l < "$review_manifest" | tr -d '[:space:]')" == 11 ]] ||
  die "review manifest must contain exactly 11 rows"
[[ "$(find "$CAMERA_DIR" -maxdepth 1 -type f -name '[0-9][0-9]-*.jpg' | wc -l | tr -d '[:space:]')" == 11 ]] ||
  die "camera directory must contain exactly 11 stage JPEGs"
[[ "$(find "$SCREENSHOT_DIR" -maxdepth 1 -type f -name '*.png' | wc -l | tr -d '[:space:]')" == 11 ]] ||
  die "screenshot directory must contain exactly 11 stage PNGs"
...
[[ "$camera_index" == 11 ]] || die "camera stage count changed while verifying"
...
stroke_camera_delta="$(pixel_difference \
  "$CAMERA_DIR/09-ink-canvas-before-stroke.jpg" \
  "$CAMERA_DIR/10-ink-stroke.jpg" \
...
[[ "$review_index" == 11 ]] || die "review stage count changed while verifying"

echo "visual acceptance verifier: PASS stages=11 reviewer=$reviewer revision=${metadata_values[0]} profile=${metadata_values[1]} manifest_sha256=${metadata_values[2]} screenshot_stroke_yavg=$stroke_screenshot_delta camera_stroke_yavg=$stroke_camera_delta camera=$CAMERA_DIR screenshots=$SCREENSHOT_DIR"
